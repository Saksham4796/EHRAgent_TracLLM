...
class PerturbationBasedAttribution(Attribution):
    ...
    def tracllm(self, question:str, contexts:list, answer:str, score_func):
        ...
        if current_nodes_scores is None:
            node_texts = [" ".join(unzip_tuples(node)[0]) for node in current_nodes]
            current_nodes_scores = self.func_map[score_func](question, node_texts, answer)
