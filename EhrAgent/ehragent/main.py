...
def extract_last_code_block(logs: list) -> str:
    """
    Extract the last code block generated by the chatbot/assistant from the logs.
    Returns the code string if found, otherwise returns None.
    """
    for entry in reversed(logs):
        ...
        role = entry.get("role")
        name = entry.get("name")
        if role not in {"assistant", "function"} and name != "chatbot":
            continue

        content = entry.get("content") or ""
        if isinstance(content, dict):
            if "cell" in content and isinstance(content["cell"], str):
                return content["cell"]
        ...
        # Sometimes the assistant returns a function_call payload
        if "function_call" in entry:
            args = entry["function_call"].get("arguments", "")
            if isinstance(args, dict):
                if "cell" in args and isinstance(args["cell"], str):
                    return args["cell"]
            if isinstance(args, str) and "cell" in args:
                try:
                    args_json = json.loads(args)
                    if isinstance(args_json, dict) and "cell" in args_json:
                        return args_json.get("cell", "")
                except Exception:
                    pass
    return None


def select_attribution_target(user_proxy, chatbot, autogen_log: str):
    """
    Prefer the last code cell for attribution; fall back to the agent's last executed code;
    otherwise use the full autogen log.
    """
    try:
        raw_messages = user_proxy.chat_messages.get(chatbot, [])
    except Exception:
        raw_messages = []

    last_code = extract_last_code_block(raw_messages)
    if last_code:
        return last_code, "code_block"

    agent_code = getattr(user_proxy, "code", None)
    if isinstance(agent_code, str) and agent_code.strip():
        return agent_code, "agent_code"

    return autogen_log, "full_log"
        ...
        try:
            if not use_tracllm:
                ...
                if needs_rerank:
                    ...
                    attribution_target, target_source = select_attribution_target(user_proxy, chatbot, autogen_log)
                    print(f"Attribution target source: {target_source}")
                    if target_source == "code_block":
                        print("Using last code block for attribution.")
                    elif target_source == "agent_code":
                        print("Using agent's last executed code for attribution.")
                    else:
                        print("Using full log for attribution (no code block found).")
                    if target_source in {"code_block", "agent_code"}:
                        print("\n" + "-" * 60)
                        print("ATTRIBUTION CODE TARGET")
                        print("-" * 60)
                        print(attribution_target)
                        print("-" * 60 + "\n")
